...
const DESCRIPTIONS = {
  color: {
    ...
    onDangerBg: "L'encre à poser sur `dangerBg`. `danger` seul n'y est pas lisible (3,39 pour 1).",
    warning: 'État de vigilance — information à confirmer, écart non bloquant.',
    warningBg: 'Fond des éléments en état de vigilance.',
...
function buildTailwind() {
  ...
  return [
    ...
    `    0%, 38% { text-decoration-color: ${colors.primary}; color: ${colors.onInfoBg}; }`,
    `    100% { text-decoration-color: transparent; color: ${colors.textMuted}; }`,
    '  }',
    '}',
    '',
    ...
    '    }',
    '    @keyframes repere-libelle {',
    `      0%, 80% { text-decoration-color: ${colors.primary}; color: ${colors.onInfoBg}; }`,
    `      100% { text-decoration-color: transparent; color: ${colors.textMuted}; }`,
    '    }',
    '  }',
    '}',
