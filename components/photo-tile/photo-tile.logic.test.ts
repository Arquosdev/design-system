...
describe('estEnTravers', () => {
  it('dit non tant que la photo est dans le sens du cadre', () => {
    strictEqual(estEnTravers(3, 4, CADRE), false, 'le cadre exact');
    // Le cas cité dans le commentaire : sur du 0,77 dans du 0,75 on rogne trois
    // pour cent, personne ne le voit.
    strictEqual(estEnTravers(770, 1000, CADRE), false);
  });

  it('dit oui sur une photo à l’horizontale', () => {
    strictEqual(estEnTravers(4, 3, CADRE), true);
    strictEqual(estEnTravers(1920, 1080, CADRE), true, 'une capture d’écran');
  });

  it('dit oui sur une photo plus étroite que le cadre', () => {
    strictEqual(estEnTravers(400, 1200, CADRE), true, 'un panoramique vertical');
  });
  ...
  it('parie sur le cas majoritaire quand les dimensions manquent', () => {
    strictEqual(estEnTravers(0, 0, CADRE), false);
    strictEqual(estEnTravers(800, 0, CADRE), false);
    strictEqual(estEnTravers(0, 600, CADRE), false);
  });
